...
class TestActivitiesEndpoints:
    ...
    def test_get_my_activities(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/v1/activities/user/me")
        assert response.status_code == 200

    def test_get_my_activities_unauthorized(self, client):
        """Test get activities without authentication."""
        response = client.get("/api/v1/activities/user/me")
    ...
    def test_get_activity_stats(self, authenticated_client, test_event):
        """Test get activity statistics."""
        response = authenticated_client.get(f"/api/v1/activities/event/{test_event.id}/stats")
        # May return 200 with stats or 404 if no data
        assert response.status_code in [200, 404]
        if response.status_code == 200:
            data = response.json()
            assert "total_distance_km" in data or "activities" in data
...
class TestProgressEndpoints:
    ...
    def test_get_my_progress(self, authenticated_client):
        ...
        response = authenticated_client.get("/api/v1/progress/user/me")
