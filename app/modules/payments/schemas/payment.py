...
class PaymentCreate(BaseModel):
    ...
    payment_method: str = Field(
        ..., max_length=50, description="Payment method (credit_card, upi, net_banking, etc.)"
    )
...
class PaymentOrderCreate(BaseModel):
    ...
    gateway: str | None = Field(
        None,
        description="Payment gateway to use (razorpay, stripe, etc.). Uses default if not specified.",
    )
...
class PaymentCaptureRequest(BaseModel):
    ...
    amount: Decimal | None = Field(
        None, ge=0, description="Amount to capture (None for full authorized amount)"
    )
...
class RefundCreate(BaseModel):
    ...
    amount: Decimal | None = Field(
        None, ge=0, description="Amount to refund (None for full refund)"
    )
...
class PaymentUpdate(BaseModel):
    ...
    status: str = Field(
        ..., max_length=50, description="Payment status (pending, completed, failed, refunded)"
    )
    transaction_id: str | None = Field(
        None, max_length=100, description="Transaction ID from payment gateway"
    )
    gateway_reference: str | None = Field(
        None, max_length=100, description="Gateway reference number"
    )
    gateway_name: str | None = Field(
        None, max_length=50, description="Payment gateway name (razorpay, stripe, etc.)"
    )
...
class PaymentResponse(BaseModel):
    ...
    class Config:
        ...
        json_encoders = {Decimal: lambda v: float(v) if v is not None else 0.0}
