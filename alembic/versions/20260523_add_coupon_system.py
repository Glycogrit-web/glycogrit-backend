...
def upgrade():
    ...
    op.add_column(
        "payments", sa.Column("original_amount", sa.Numeric(precision=10, scale=2), nullable=True)
    )
    op.add_column(
        "payments",
        sa.Column(
    ...
    op.add_column(
        "payments", sa.Column("idempotency_key", sa.String(length=100), nullable=True, unique=True)
    )

    # Constraints for payment discount validation
    op.create_check_constraint(
        "ck_payment_discount_non_negative", "payments", "discount_amount >= 0"
    )
    op.create_check_constraint(
    ...
    op.create_check_constraint(
        "ck_tier_reserved_spots_non_negative", "event_registration_tiers", "reserved_spots >= 0"
    )
...
def downgrade():
    ...
    op.drop_constraint("ck_tier_capacity_limit", "event_registration_tiers", type_="check")
    op.drop_constraint(
        "ck_tier_reserved_spots_non_negative", "event_registration_tiers", type_="check"
    )
