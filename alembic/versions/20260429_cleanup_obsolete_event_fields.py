...
def downgrade():
    ...
    op.add_column(
        "events", sa.Column("sync_enabled", sa.Boolean(), server_default="true", nullable=False)
    )
    ...
    op.add_column(
        "events",
        sa.Column("uses_tier_system", sa.Boolean(), server_default="false", nullable=False),
    )

    # Reward/goodie fields
    op.add_column(
        "events", sa.Column("goodies", postgresql.JSONB(astext_type=sa.Text()), nullable=True)
    )
    op.add_column(
        "events", sa.Column("rewards", postgresql.JSONB(astext_type=sa.Text()), nullable=True)
    )

    # Payment/pricing fields
    op.add_column(
        "events", sa.Column("registration_fee", sa.Numeric(precision=10, scale=2), nullable=True)
    )
    op.add_column(
        "events",
        sa.Column("requires_payment", sa.Boolean(), server_default="false", nullable=False),
    )
