...
down_revision: Union[str, None] = '0d45922d16b5'
...
def upgrade() -> None:
    ...
    op.execute("DO $$ BEGIN CREATE TYPE rewardstatus AS ENUM ('pending_details', 'pending_shipment', 'label_generated', 'pickup_scheduled', 'shipped', 'in_transit', 'out_for_delivery', 'delivered', 'cancelled', 'rto_initiated', 'rto_delivered'); EXCEPTION WHEN duplicate_object THEN null; END $$;")
    op.execute("DO $$ BEGIN CREATE TYPE rewardtype AS ENUM ('medal', 'tshirt', 'certificate', 'trophy', 'custom'); EXCEPTION WHEN duplicate_object THEN null; END $$;")
    op.execute("DO $$ BEGIN CREATE TYPE shiprocketorderstatus AS ENUM ('pending', 'created', 'label_generated', 'pickup_scheduled', 'manifested', 'failed'); EXCEPTION WHEN duplicate_object THEN null; END $$;")
    ...
    op.create_index(op.f('ix_user_rewards_registration_id'), 'user_rewards', ['registration_id'], unique=False)
    op.create_foreign_key('fk_user_rewards_registration_id', 'user_rewards', 'registrations', ['registration_id'], ['id'])
    ...
    op.execute("ALTER TABLE user_rewards ALTER COLUMN status TYPE rewardstatus USING status::text::rewardstatus")
    op.execute("ALTER TABLE user_rewards ALTER COLUMN reward_type TYPE rewardtype USING reward_type::text::rewardtype")

    # Step 4: Add new Shiprocket tracking fields
    op.add_column('user_rewards', sa.Column('item_weight', sa.Numeric(precision=10, scale=2), nullable=True))
    op.add_column('user_rewards', sa.Column('item_length', sa.Numeric(precision=10, scale=2), nullable=True))
    op.add_column('user_rewards', sa.Column('item_breadth', sa.Numeric(precision=10, scale=2), nullable=True))
    op.add_column('user_rewards', sa.Column('item_height', sa.Numeric(precision=10, scale=2), nullable=True))
    ...
    op.add_column('user_rewards', sa.Column('current_location', sa.String(length=200), nullable=True))
    op.add_column('user_rewards', sa.Column('last_tracking_update', sa.DateTime(timezone=True), nullable=True))
    ...
    op.add_column('user_rewards', sa.Column('status_history', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    ...
    op.create_index(op.f('ix_user_rewards_shiprocket_awb'), 'user_rewards', ['shiprocket_awb'], unique=False)
    ...
    """)
    op.create_index(op.f('ix_shiprocket_orders_event_id'), 'shiprocket_orders', ['event_id'], unique=False)
    op.create_index(op.f('ix_shiprocket_orders_order_reference'), 'shiprocket_orders', ['order_reference'], unique=True)
    op.create_index(op.f('ix_shiprocket_orders_shiprocket_awb'), 'shiprocket_orders', ['shiprocket_awb'], unique=False)
    op.create_index(op.f('ix_shiprocket_orders_shiprocket_order_id'), 'shiprocket_orders', ['shiprocket_order_id'], unique=True)
    op.create_index(op.f('ix_shiprocket_orders_shiprocket_shipment_id'), 'shiprocket_orders', ['shiprocket_shipment_id'], unique=False)
    op.create_index(op.f('ix_shiprocket_orders_status'), 'shiprocket_orders', ['status'], unique=False)
    op.create_index(op.f('ix_shiprocket_orders_user_id'), 'shiprocket_orders', ['user_id'], unique=False)
    op.create_index(op.f('ix_shiprocket_orders_user_reward_id'), 'shiprocket_orders', ['user_reward_id'], unique=True)

    # Step 6: Create shiprocket_config table
    op.create_table('shiprocket_config',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('email', sa.String(length=255), nullable=False),
        sa.Column('encrypted_password', sa.Text(), nullable=False),
        sa.Column('access_token', sa.Text(), nullable=True),
        sa.Column('token_expires_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('default_pickup_location', sa.String(length=200), nullable=False, server_default='Primary'),
        sa.Column('default_length', sa.Numeric(precision=10, scale=2), nullable=False, server_default='15.0'),
        sa.Column('default_breadth', sa.Numeric(precision=10, scale=2), nullable=False, server_default='15.0'),
        sa.Column('default_height', sa.Numeric(precision=10, scale=2), nullable=False, server_default='10.0'),
        sa.Column('default_weight', sa.Numeric(precision=10, scale=2), nullable=False, server_default='0.5'),
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_shiprocket_orders_shiprocket_shipment_id'), table_name='shiprocket_orders')
    ...
    op.execute("CREATE TYPE goodiestatus AS ENUM ('pending_details', 'pending_shipment', 'shipped', 'delivered', 'cancelled')")
    op.execute("CREATE TYPE goodietype AS ENUM ('medal', 'tshirt', 'certificate', 'trophy', 'custom')")
    ...
    op.alter_column('user_rewards', 'reward_type', new_column_name='goodie_type', type_=sa.Enum('medal', 'tshirt', 'certificate', 'trophy', 'custom', name='goodietype'))
    ...
    op.add_column('user_rewards', sa.Column('requires_shipping_enum', sa.Enum('true', 'false', name='boolean_enum'), nullable=True))
    op.add_column('user_rewards', sa.Column('is_unlocked_enum', sa.Enum('true', 'false', name='boolean_enum_unlocked'), nullable=True))
    op.add_column('user_rewards', sa.Column('is_verified_enum', sa.Enum('true', 'false', name='boolean_enum_verified'), nullable=True))
    ...
    op.execute("ALTER TABLE user_rewards ALTER COLUMN status TYPE goodiestatus USING status::text::goodiestatus")
