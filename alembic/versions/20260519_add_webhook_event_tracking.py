...
def upgrade():
    ...
    # Composite indexes
    op.create_index(
        "idx_webhook_gateway_event", "webhook_events", ["gateway_name", "gateway_event_id"]
    )

    op.create_index("idx_webhook_processing", "webhook_events", ["processed", "received_at"])

    op.create_index(
        "idx_webhook_payment_lookup", "webhook_events", ["gateway_payment_id", "gateway_name"]
    )
