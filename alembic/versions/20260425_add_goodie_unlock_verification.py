...
def upgrade():
    ...
    op.add_column(
        "user_goodies", sa.Column("unlocked_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "user_goodies", sa.Column("verified_at", sa.DateTime(timezone=True), nullable=True)
    )

    # Create indexes for new fields
    op.create_index(
        op.f("ix_user_goodies_is_unlocked"), "user_goodies", ["is_unlocked"], unique=False
    )

    # Create foreign key constraints
    op.create_foreign_key(
        "fk_user_goodies_unlocked_by_admin",
        "user_goodies",
        "users",
    ...
    op.create_foreign_key(
        "fk_user_goodies_verified_by_admin",
        "user_goodies",
        "users",
