...
def upgrade() -> None:
    ...
    op.add_column(
        "user_rewards", sa.Column("certificate_url", sa.String(length=500), nullable=True)
    )
    op.add_column(
        "user_rewards", sa.Column("certificate_number", sa.String(length=100), nullable=True)
    )
    ...
    op.add_column(
        "user_rewards",
        sa.Column("download_limit", sa.Integer(), nullable=False, server_default="10"),
    )
    ...
    op.create_index(
        "idx_user_rewards_certificate", "user_rewards", ["reward_type", "certificate_url"]
    )
    op.create_unique_constraint(
        "uq_user_rewards_certificate_number", "user_rewards", ["certificate_number"]
    )
    op.create_index(
        "idx_user_rewards_downloads", "user_rewards", ["download_count", "download_limit"]
    )
