...
def upgrade():
    ...
    op.add_column("registrations", sa.Column("last_payment_status", sa.String(20), nullable=True))

    # Add last_payment_amount - amount of most recent payment attempt
    op.add_column(
        "registrations", sa.Column("last_payment_amount", sa.Numeric(10, 2), nullable=True)
    )

    # Add last_payment_date - timestamp of most recent payment attempt
    op.add_column(
        "registrations", sa.Column("last_payment_date", sa.TIMESTAMP(timezone=True), nullable=True)
    )

    # Create index for querying by payment status
    op.create_index(
        "ix_registrations_last_payment_status", "registrations", ["last_payment_status"]
    )
