...
down_revision: str | None = "add_payment_tracking"
...
def upgrade() -> None:
    ...
    op.add_column(
        "event_activities", sa.Column("activity_type", sa.String(length=50), nullable=True)
    )
    ...
    op.create_index(
        op.f("ix_registrations_event_activity_id"),
        "registrations",
        ["event_activity_id"],
        unique=False,
    )
    op.create_foreign_key(
        "fk_registrations_event_activity_id",
        "registrations",
        "event_activities",
        ["event_activity_id"],
        ["id"],
    ...
    op.execute(
        "UPDATE registrations SET event_activity_id = event_category_id WHERE event_category_id IS NOT NULL"
    )
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["event_id"],
            ["events.id"],
            name="event_activity_types_event_id_fkey",
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name="event_activity_types_pkey"),
    )
    op.create_index(
        "ix_event_activity_types_event_id", "event_activity_types", ["event_id"], unique=False
    )
    op.create_index(
        "ix_event_activity_types_activity_type",
        "event_activity_types",
        ["activity_type"],
        unique=False,
    )
    ...
    op.add_column(
        "events", sa.Column("event_type", sa.VARCHAR(length=50), autoincrement=False, nullable=True)
    )
    ...
    op.add_column(
        "registrations",
        sa.Column("event_category_id", sa.INTEGER(), autoincrement=False, nullable=True),
    )
    op.create_index(
        "ix_registrations_event_category_id", "registrations", ["event_category_id"], unique=False
    )
    op.create_foreign_key(
    ...
    op.execute(
        "UPDATE registrations SET event_category_id = event_activity_id WHERE event_activity_id IS NOT NULL"
    )
