...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        ),  # google_fit, apple_health, nike_run_club
        ...
        sa.Column(
            "provider_data", sa.Text(), nullable=True
        ),  # JSON string with provider-specific data
    ...
    op.create_index(
        "idx_fitness_tracker_user_provider",
        "fitness_tracker_connections",
        ["user_id", "provider"],
        unique=True,
    )
    ...
    op.add_column(
        "challenge_activities", sa.Column("source_provider", sa.String(50), nullable=True)
    )  # strava, google_fit, etc
    op.add_column(
        "challenge_activities", sa.Column("external_activity_id", sa.String(255), nullable=True)
    )
    op.create_index(
        "idx_challenge_activity_external",
        "challenge_activities",
        ["source_provider", "external_activity_id"],
    )

    # Add completion tracking to user_challenge_progress
    op.add_column(
        "user_challenge_progress", sa.Column("completion_status", sa.String(50), nullable=True)
    )  # failed, completed, exceeded, outstanding
    op.add_column(
        "user_challenge_progress", sa.Column("completion_percentage", sa.Integer(), default=0)
    )
    op.add_column(
    ...
    op.add_column(
        "user_challenge_progress", sa.Column("badge_earned", sa.String(100), nullable=True)
    )
    ...
    op.add_column(
        "events", sa.Column("auto_completed_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "events", sa.Column("sync_enabled", sa.Boolean(), default=True)
    )  # Enable/disable auto-sync
    op.add_column(
        "events", sa.Column("completion_criteria", JSONB, nullable=True)
    )  # {"min_distance": 100, "min_activities": 10}
